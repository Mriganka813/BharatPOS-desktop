...
class _BluetoothPrinterListState extends State<BluetoothPrinterList> {
  ...
  //   });
  // }
  _scan(PrinterType type, {bool isBle = true}) {
    // Find printers
    PrinterManager.instance.discovery(type: type, isBle: isBle).listen((device) {
      // locator<GlobalServices>().infoSnackBar(device.name);
      print("device adding");
      devices.add(device);
      setState(() {
        print("setting state");
  ...
  Future<void> printKot() async {
    ...
    PrinterManager.instance.stateBluetooth.listen((status) {
      locator<GlobalServices>().infoSnackBar(status.toString());
    });
    // if (conecctionStatus) {
      List<int> ticket = await kotTicket();
      PrinterManager.instance.send(type: defaultPrinterType, bytes: ticket);
                            ...
                            return ListTile(
                              onTap: () async {
                                ...
                                if (widget.args.bluetoothArgs != null &&
                                  ...
                                  isPrinted = true;
                                  setState(() {});
                                  await _connectDevice(devices[position], defaultPrinterType);
                                  printKot();
                              ...
                              title: Text(devices[position].name),
                              // subtitle: Text(devices[position].address!),
                            );
                          },
