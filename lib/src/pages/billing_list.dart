...
// Iterate through kot.items in reverse order
  ...
  void _view57mmPdf(String kotId) async {
    User user = User();
    try {
    ...
    List<Map<String, dynamic>> list = await getKotData(kotId);
    ...
    await PdfKotUI.generate57mmKot(
        ...
        date: DateTime.now(),
        invoiceNum: date);
    Kot _kot = Kot(kotId: kotId,items: []);
    List<Item> kotItems = [];
  ...
  void _view80mmPdf(String kotId) async {
    User user = User();
    try {
      final res = await UserService.me();
      if ((res.statusCode ?? 400) < 300) {
  ...
  _showNewDialog(Order order) async {
    if (order.tableNo == "-1" || order.tableNo == "null") {
